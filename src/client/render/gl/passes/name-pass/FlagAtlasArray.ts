/**
 * FlagAtlasArray — runtime TEXTURE_2D_ARRAY of original heraldic nameplates.
 *
 ...
 * on demand as players arrive, keyed by the original URL so identical factions
 * share a layer. URLs only seed original heraldry; no modern flags, remote
 * artwork or brand-logo images are fetched. Square cells receive royal crowns.
 *
...
import { heraldicSvg } from "../../../../theme/Heraldry";
...
export class FlagAtlasArray {
  ...
  request(url: string): void {
    ...
    img.src = `data:image/svg+xml,${encodeURIComponent(
      heraldicSvg(url, this.cellW === this.cellH),
    )}`;
  }
