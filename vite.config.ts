...
function crownFrontLocal(): Plugin {
  return {
    name: "crownfront-local",
    load(id) {
      if (id.replace(/\\/g, "/").includes("/proprietary/")) {
        throw new Error(
          "CrownFront cannot bundle restricted proprietary assets.",
        );
      }
    },
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        ...
        const pathname = new URL(requested, "http://localhost").pathname;
        if (pathname === "/source") {
          void serveCrownFrontSource(__dirname, res).catch(next);
        } else if (pathname.startsWith("/local-api/")) {
          res.statusCode = 503;
          res.setHeader("Content-Type", "application/json");
          res.end(
            JSON.stringify({
              error:
                "Online account services are not part of CrownFront. Use local solo or multiplayer.",
            }),
          );
        } else if (
          pathname.startsWith("/proprietary/") ||
          pathname.includes("/proprietary/")
        ) {
          res.statusCode = 404;
          res.end("Restricted assets are not included in CrownFront.");
        } else {
          next();
        }
      });
...
export default defineConfig(({ mode }) => {
  ...
  const syncHashedPublicAssets = (): Plugin => ({
    ...
    async closeBundle() {
      ...
      // only. Vite's bundle output (assets/...) doesn't, so it's
      // merged in after.
      ...
      await writeCrownFrontSource(
        __dirname,
        path.join(outDir, "source.tar.gz"),
      );
    },
  });
