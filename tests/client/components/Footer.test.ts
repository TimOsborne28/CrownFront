...
describe("page-footer version line", () => {
  ...
  it("keeps the derivative version without Steam promotion in a desktop shell", async () => {
    ...
    await vi.waitFor(async () => {
      ...
      const line = footer.querySelector(".footer-version");
      expect(line?.textContent?.trim()).toBe(gameVersion);
      expect(footer.querySelector("steam-wishlist-button")).toBeNull();
    });
  });

  it("retains the upstream attribution and links to the modified local source", async () => {
    await mount();
    expect(footer.textContent).toContain("© OpenFront and Contributors");
    expect(footer.querySelector('a[href="/source"]')).not.toBeNull();
    expect(footer.querySelector('a[href^="https://"]')).toBeNull();
  });
