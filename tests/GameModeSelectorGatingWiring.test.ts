...
describe("grid placement", () => {
  ...
  it("keeps local play actions available without a public-lobby loading panel", () => {
    expect(heroSlot()).toBeNull();
    expect(section().className).toContain("crownfront-live-lobbies");
    expect(
      selector.querySelector(".crownfront-primary-actions"),
    ).not.toBeNull();
  });
  ...
  it("places available FFA and team lobbies in visible card slots", async () => {
    await pushLobbies({
      ffa: [publicLobby("ffa-1")],
      team: [publicLobby("t")],
    });
    expect(section().querySelectorAll(".crownfront-lobby-card")).toHaveLength(
      2,
    );
  });

  it("gives each lobby mode its own responsive card slot", async () => {
    await pushLobbies({
      ffa: [publicLobby("ffa-1")],
      team: [publicLobby("t")],
