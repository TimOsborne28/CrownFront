...
describe("background music", () => {
  it("is a single looping track, not a playlist", () => {
    const music = find("lute.wav");
    expect(music).toBeDefined();
    expect(music.loop).toBe(true);
    expect(howlInstances.filter((h) => h.src.includes("lute.wav")).length).toBe(
      1,
    );
  });
  ...
  it("streams instead of waiting for the whole file to decode", () => {
    ...
    // before the first note. Ambience and cues stay on Web Audio, so this
    // has to stay specific to the music track.
    expect(find("lute.wav").html5).toBe(true);
  });
  ...
  it("follows the music slider through the mixer", () => {
    ...
    expect(
      find("lute.wav").volumes[find("lute.wav").volumes.length - 1],
...
describe("ambience", () => {
  ...
  it("crossfades when the nearest structure changes", () => {
    ...
    expect(find("forge.wav").play).toHaveBeenCalled();
  });
