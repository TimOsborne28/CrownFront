...
import { cartographicPreview } from "./CartographicPreview";
...
export class MapDisplay extends LitElement {
  ...
  private async loadMapData() {
    ...
    try {
      ...
      const mapValue = GameMapType[key as keyof typeof GameMapType];
      const data = terrainMapFileLoader.getMapData(mapValue);
      const manifest = await data.manifest();
      const previewUrl = await cartographicPreview(key, data, manifest);
      if (generation !== this.loadGeneration || key !== this.mapKey) return;
  ...
  render() {
    return html`
      <div
        ...
        class="crownfront-map-card w-full h-full p-3 flex flex-col items-center justify-between rounded border cursor-pointer transition-all duration-200 active:scale-95 gap-3 group ${this
          .selected
      ...
      >
        ${this.isLoading
          ...
          : this.previewUrl
            ? html`<div
                class="w-full aspect-[2/1] relative overflow-hidden rounded border border-[#a68b51]/50 bg-[#292319]"
              >
                <img
                  src="${this.previewUrl}"
                  ...
                  class="w-full h-full object-contain ${this.selected
                    ? "opacity-100"
