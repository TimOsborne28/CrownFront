...
// King's Road: a continuous paved route, never railway tracks or cross-ties.
// The centerline and width still come from the unchanged connectivity data.
...
void main() {
  ...
  // --- Color output ---
  vec3 bridgeColor = vec3(0.46, 0.32, 0.18);
  ...
  if (hitRail) {
    ...
    if (detailMode) {
      vec2 paving = fract(vWorldPos * 3.0 + vec2(mod(floor(vWorldPos.y * 3.0), 2.0) * 0.5, 0.0));
      float mortar = step(0.84, paving.x) + step(0.84, paving.y);
      railColor *= 1.0 - min(mortar, 1.0) * 0.24;
    }
