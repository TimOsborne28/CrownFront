...
CrownFront's active structure, mobile-unit, effect and status atlases are
original SVGs. Rebuild them and their matching HUD icons with
`node scripts/generate-crownfront-art.mjs` (dependency-free; no source artwork
or network access). The generator preserves all atlas column/row ordering,
logical world dimensions, grayscale heraldry bands and effect frame counts.
`fx-atlas-meta.json` remains in logical pixels; SVG raster dimensions are
supersampled by four without changing UV ratios or effect sizes.

Both `TerrainPass` and map-picker overview previews use `buildTerrainRGBA`.
Live terrain-rectangle uploads must call `encodeCartographicTile` with absolute
map coordinates so engraved marks do not jump after terrain changes.
Never apply cartographic color transformations to player/affiliation palettes.

`FlagAtlasArray` uses faction/crown URLs only as stable identity keys for
`theme/Heraldry.ts`. It generates original shield/crown SVGs locally rather
than fetching national flags, external cosmetics or upstream logo artwork.
`IconProgram` replaces the eight economy/unit emoji cells with matching original
SVG icons before uploading its texture, preserving every communication token.
