...
export class EventsDisplay extends LitElement implements Controller {
  ...
  private resolveParams(
    ...
    if (typeof params.unit === "string") {
      params.unit = unitDisplayName(params.unit);
    }
  ...
  onUnitIncomingEvent(event: UnitIncomingUpdate) {
    ...
    if (unitView) {
      const name = unitView.owner().displayName();
      description =
        event.messageType === MessageType.NAVAL_INVASION_INBOUND
          ? translateText("events_display.naval_invasion_inbound", {
              name,
              troops: renderTroops(unitView.troops()),
            })
          : translateText(
              event.messageType === MessageType.MIRV_INBOUND
                ? "events_display.plague_volley_inbound"
                : "events_display.doom_weapon_inbound",
              {
                name,
                unit: unitDisplayName(unitView.type()),
              },
            );
    }
