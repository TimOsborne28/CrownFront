...
void main() {
  ...
  // Print optional map details in the same ink-and-parchment palette; retain
  // every layer's position, transparency, visibility and destruction mask.
  float value = dot(layer.rgb, vec3(0.299, 0.587, 0.114));
  vec3 ink = isLand ? vec3(0.31, 0.28, 0.19) : vec3(0.24, 0.38, 0.37);
  vec3 paper = isLand ? vec3(0.87, 0.81, 0.64) : vec3(0.57, 0.70, 0.67);
  fragColor = vec4(mix(ink, paper, value), layer.a * uAlpha);
}
