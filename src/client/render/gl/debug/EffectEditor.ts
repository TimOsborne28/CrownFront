...
export function buildEffectEditor(
  ...
  for (const effectType of EFFECT_TYPES) {
    ...
    const folder = gui.addFolder(translateText(`effects.type.${effectType}`));
      ...
      fieldCtrls.push([
        ...
        folder
          .add(
            ...
            "nukeType",
            Object.fromEntries(
              NUKE_EXPLOSION_TYPES.map((type) => [
                translateText(`effects.nukeType.${type}`),
                type,
              ]),
            ),
          )
          .name(translateText("effects.type.nukeExplosion"))
          .onChange(apply),
      ]);
    }
