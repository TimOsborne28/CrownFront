...
vi.mock("../../../../src/client/Utils", () => ({
  ...
  renderNumber: vi.fn(),
  renderTroops: vi.fn((troops: number) => String(troops)),
...
describe("EventsDisplay handlers", () => {
  ...
  describe("tick", () => {
    it("dispatches queued updates through updateMap", () => {
      ...
      expect(events().map((e) => e.description)).toEqual([
        "hello world",
        'events_display.doom_weapon_inbound {"name":"Stranger","unit":"unit_type.atom_bomb"}',
      ]);
    });
  ...
  describe("onUnitIncomingEvent", () => {
    ...
    it("keeps the troop count in translated naval invasion warnings", () => {
      ed.onUnitIncomingEvent({
        ...incoming(1),
        messageType: MessageType.NAVAL_INVASION_INBOUND,
      } as never);
      expect(events()[0].description).toBe(
        'events_display.naval_invasion_inbound {"name":"Stranger","troops":"2500"}',
      );
    });

    it("retains the urgent warning for a plague volley", () => {
      game.unit = () => ({
        isActive: () => true,
        owner: () => stranger,
        type: () => UnitType.MIRV,
      ...
      ed.onUnitIncomingEvent({
        ...incoming(1),
        messageType: MessageType.MIRV_INBOUND,
      } as never);
      expect(events()[0].description).toBe(
        'events_display.plague_volley_inbound {"name":"Stranger","unit":"unit_type.mirv"}',
      );
    });

    it("preserves the original warning when the incoming unit is unavailable", () => {
      game.unit = () => undefined;
      ed.onUnitIncomingEvent(incoming(1) as never);
    ...
    it.each([UnitType.Warship, UnitType.TransportShip, UnitType.MIRVWarhead])(
      "translates unit parameters without mutating the simulation update: %s",
      (unit) => {
        const params = { unit };
        ...
        expect(events()[0].description).toContain("unit_type.");
        expect(params.unit).toBe(unit);
      },
    );
  });
});
