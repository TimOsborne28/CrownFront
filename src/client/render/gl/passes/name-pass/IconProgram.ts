...
export const MEDIEVAL_EMOJI_ICONS = {
  "☢️": "NukeIconWhite.svg",
  "💰": "GoldCoinIcon.svg",
  "⚓": "PortIcon.svg",
  "⛵": "BoatIconWhite.svg",
  "🏡": "CityIconWhite.svg",
  "🛡️": "ShieldIconWhite.svg",
  "🏭": "FactoryIconWhite.svg",
  "🚂": "CaravanIcon.svg",
} as const;

async function medievalEmojiCanvas(
  source: HTMLImageElement,
): Promise<HTMLCanvasElement> {
  const canvas = document.createElement("canvas");
  canvas.width = emojiAtlasMeta.width;
  canvas.height = emojiAtlasMeta.height;
  const ctx = canvas.getContext("2d")!;
  ctx.drawImage(source, 0, 0);
  await Promise.all(
    Object.entries(MEDIEVAL_EMOJI_ICONS).map(async ([emoji, filename]) => {
      const index =
        emojiAtlasMeta.emojis[emoji as keyof typeof emojiAtlasMeta.emojis];
      const { cellSize, cols } = emojiAtlasMeta;
      const x = (index % cols) * cellSize;
      const y = Math.floor(index / cols) * cellSize;
      // Keep wire emoji values and layout; only replace modern unit artwork.
      ctx.clearRect(x, y, cellSize, cellSize);
      const image = new Image();
      image.crossOrigin = "anonymous";
      image.src = assetUrl(`images/${filename}`);
      await image.decode();
      ctx.drawImage(image, x + 8, y + 8, cellSize - 16, cellSize - 16);
    }),
  );
  return canvas;
}
...
export class IconProgram {
  ...
  private loadEmojiAtlas(): void {
    ...
    img.onload = async () => {
      ...
      try {
        source = await medievalEmojiCanvas(img);
      } catch (error) {
        console.warn("Failed to prepare heraldic emoji atlas:", error);
        return;
      }
