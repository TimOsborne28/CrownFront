...
import keepIcon from "../../resources/images/CityIconWhite.svg";
...
describe("EffectScene", () => {
  ...
  it("spans a structure's gradient across the shared keep icon once", async () => {
    const el = await render(effect("structures", gradient));
    expect(el.querySelector("image[data-glyph]")?.getAttribute("href")).toBe(
      keepIcon,
    );
    expect(el.querySelector("[data-fill]")?.getAttribute("fill")).toMatch(
      /^url\(#/,
