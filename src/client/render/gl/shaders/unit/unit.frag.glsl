...
// Wildfire colors; cadence and phase still encode the same projectile states.
...
void main() {
  ...
  vec3 color;
  if (gray > 0.9) {
    // Parchment sailcloth / hot projectile cores. Heraldic bands below still
    // carry ownership, attacks, retreat blink and equipped effects.
    color = vec3(0.969, 0.910, 0.769);
  } else if (gray > 0.6) {
