import renderDefaults from "../src/client/render/gl/render-settings.json";
import {
  encodeTerrainTile,
  hexToRgb,
...
describe("Impassable Terrain", () => {
  ...
  test("encodeTerrainTile renders impassable as the map background colour", () => {
    ...
    // Must match the configured clear colour in Renderer.ts drawBaseLayer().
    expect([...out]).toEqual([
      ...hexToRgb(renderDefaults.terrain.backgroundColor)!,
      255,
    ]);
  });
  ...
  test("encodeTerrainTile renders plains normally (not background)", () => {
    ...
    expect([...out].slice(0, 3)).toEqual(
      hexToRgb(renderDefaults.terrain.plainsColor),
    );
    expect([...out].slice(0, 3)).not.toEqual(
      hexToRgb(renderDefaults.terrain.backgroundColor),
    );
  });
