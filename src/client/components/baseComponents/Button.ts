...
export class OButton extends LitElement {
  ...
  private readonly BASE =
    "font-bold uppercase tracking-wider rounded border border-transparent " +
    "transition-colors duration-200 " +
    "focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-malibu-blue text-center whitespace-normal break-words leading-tight overflow-hidden relative " +
    "disabled:cursor-not-allowed disabled:hover:translate-y-0 disabled:opacity-70";

  private variantClasses(): string {
    switch (this.variant) {
      case "primary":
        return "crownfront-action crownfront-action-primary disabled:bg-gray-600 disabled:text-gray-300";
      case "secondary":
        return "crownfront-action crownfront-action-secondary disabled:bg-gray-800 disabled:text-gray-400";
      case "danger":
